...
import { Link, redirect, useLoaderData, useActionData } from "remix";
...
function Checkout() {
  ...
  const product = data.map((product, index) => 
    <div className="product" key={index}>
      <div className={`image image-${product?.product?.slug}`}><img src={`/assets/cart/image-${product?.product?.slug}.jpg`} alt="" /></div>
      <div className="name-price">
        <div className="product-name">{product?.product?.shortName}</div>
        <div className="price">$ {product?.product?.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</div>
      </div>
  ...
  return (
    <div className={`checkout-page ${actionData?.submitted ? "open-checkout-overlay" : ""}`} >
      ...
      <div className={`checkout-form`}>
        <div className={`purchase-modal ${actionData?.submitted ? "purchase-success" : ""}`} >
          <div className="mask">
            <div className="success-tick"><img src="/assets/shared/desktop/tick.svg" alt="" /></div>
            ...
            <div className="product-ordered">
              <div className="products-mask">
                <div className="products">
                  <div className="image"><img src={`/assets/cart/image-${data[0]?.product?.slug}.jpg`} alt="" /></div>
        ...
        <form method="post" className="form" target="_self">
              ...
              <div className="payment-details">
                ...
                <div className="payment-method">
                  <div className="method">
                    <input type="radio" name="payment-method" id="e-money" autoComplete="off" required  onChange={handleChenge} value={"e-money"}/>
                    <label htmlFor="e-money"><span className="checked"></span> e-Money</label>
                  </div>
                  <div className="method">
                    <input type="radio" name="payment-method" id="cash" autoComplete="off" required onChange={handleChenge} value={"cash"}/>                    
                    <label htmlFor="cash"><span className="checked"></span> Cash on Delivery </label>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              {cash ?
                <div className="payment-method-mask">
                  ...
                  <p className="cash-desc">The ‘Cash on Delivery’ option enables you to pay in cash when our delivery courier arrives at your residence. Just make sure your address is correct so that your order will not be cancelled.</p>
                </div>
                :
                <div className="payment-method-mask">
                  <div className="input-wrapper">
          ...
          <div className="summary">
            ...
            <div className="price-mask">
              <p>Total</p>
              <p className="price">$ {total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</p>
            </div>

            <div className="price-mask">
              <p>shipping</p>
              <p className="price">$ {shipping.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</p>
            </div>

            <div className="price-mask">
              <p>VAT (INCLUDED)</p>
              <p className="price">$ {vat.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</p>
            </div>

            <div className="price-mask grand-total">
              <p>GRAND TOTAL</p>
              <p className="price grand-price">$ {grandTotal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</p>
            </div>
            <button type="submit" className="pay">CONTINUE & PAY</button>
          </div>                        
        </form>
      </div>
    </div>
