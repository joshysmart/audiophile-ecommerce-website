import { Link, useParams, useLoaderData, redirect, useActionData } from "remix"
...
export const loader = async ({request, params}) => {
  const data = {
    products: await db.product.findMany({
      select: {id: true, new: true, name: true, description: true, price: true, features: true, includes: true, others: true, slug: true, category: true},
      where: {
        slug: {
...
export const action = async ({request, params}) => {
  ...
  switch(action) {
    ...
    default: {
      const data = {
        products: await db.product.findMany({
          select: {id: true, name: true, price: true, slug: true, shortName: true},
          where: {
            slug: {
              equals: `${params.productId}`
            }
    ...
    <div className="product-details">
      ...
      <div className="product">
        ...
        <div className="product-desc">
          {product.new ? <div className="new"><p className="orange-text">New Product</p></div> : ""}
      ...
      <div className="features">
        ...
        <div className="content">
          <h3>in the box</h3>
          <div className="items">
            {product.includes.map((item, index) => <p key={index}><span className="orange-text">{item.quantity}x</span> {item.item}</p>)}
          </div>
        </div>
      </div>
        ...
        <div className="products">
          {product.others.map((others, index) => 
            <div className={`other-product product-${index}`} key={index}>
              ...
              <h4>{others.name}</h4>
              <Link to={`/productdetails/${others.slug}`} target={"_self"}><button className="see-product">See Product</button></Link>
            </div>
          )}
        </div>
