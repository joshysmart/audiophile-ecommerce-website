...
function Speakers() {
  ...
  const product = speakers.map((speaker, index) => 
    <div className={`product product-${index}`} key={index}>
      ...
      <div className="description">
        {speaker.new ? <div className="new"><p className="orange-text">New Product</p></div> : ""}
        <h2 className="product-name">{speaker.name}</h2>
        <p className="product-desc">{speaker.description}</p>
        <Link to={`/productdetails/${speaker.slug}`} target={"_self"}><button className="see-product">See Product</button></Link>
      </div>
    </div>    
  );
  ...
  return (
    <div className="category">
      ...
      <div className="products speakers">
        {product}
      </div>
    </div>
  )
